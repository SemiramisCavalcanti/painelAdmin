
<body class="hold-transition sidebar-mini layout-fixed">
    <div class="wrapper">


        <!-- Preloader -->
        <div class="preloader">
            <img src="painel/dist/img/AdminLTELogo.png" alt="AdminLTELogo" height="30" width="60">
        </div>

        <!-- Navbar -->
        <nav class="main-header navbar navbar-expand navbar-white navbar-light">
            <ul class="navbar-nav">
                <!-- Left navbar links -->
                <li class="nav-item">
                    <a class="nav-link" data-widget="pushmenu" href="#" role="button"><i class="fas fa-bars"></i></a>
                </li>
                <li class="nav-item d-none d-sm-inline-block">
                    <a href="?pg=dashboard" class="nav-link">Home</a>
                </li>
                <li class="nav-item d-none d-sm-inline-block">
                    <a href="?pg=produtos" class="nav-link">Produto</a>
                </li>
                <li class="nav-item d-none d-sm-inline-block">
                    <a href="?pg=sobre" class="nav-link">Sobre</a>
                </li>
                <li class="nav-item d-none d-sm-inline-block">
                    <a href="?pg=contato" class="nav-link">Contato</a>
                </li>
                <li class="nav-item d-none d-sm-inline-block">
                    <a href="?pg=login" class="nav-link">Trocar de Usuário</a>
                </li>
                <li class="nav-item d-none d-sm-inline-block">
                    <a href="?pg=inicial" class="nav-link">Sair</a>
                </li>
            </ul>

            <!-- Right navbar links -->
            <ul class="navbar-nav ml-auto">


                <li class="nav-item">
                    <a class="nav-link" data-widget="fullscreen" href="#" role="button">
                        <i class="fas fa-expand-arrows-alt"></i>
                    </a>
                </li>

            </ul>
        </nav>
        <!-- /.navbar -->



        <!-- Main Sidebar Container -->
        <aside class="main-sidebar sidebar-dark-primary elevation-4">
            <!-- Brand Logo -->
            <a href="?pg=dashboard" class="brand-link">
                <img src="https://i.pinimg.com/736x/d8/ba/44/d8ba441ca1a2b0b77d82f358a7835a9e.jpg" alt="AdminLTE Logo" class="brand-image img-circle elevation-3" style="opacity: .8">
                <span class="brand-text font-weight-light">Painel Administrativo</span>
            </a>

            <!-- Sidebar -->
            <div class="sidebar">
                <!-- Sidebar user panel (optional) -->
                <div class="user-panel mt-3 pb-3 mb-3 d-flex">
                    <div class="image">
                        <img src="https://media-exp1.licdn.com/dms/image/C4E03AQFmw80XsC4bIg/profile-displayphoto-shrink_200_200/0/1566035885632?e=1617235200&v=beta&t=C6MOSndIRka_Sd0gYfyo6MxI724yycQGWdndTH64O4Q" class="img-circle elevation-2" alt="User Image">
                    </div>
                    <div class="info">
                        <a href="#" class="d-block">Semiramis Cavalcanti</a>
                    </div>
                </div>


                <!-- Sidebar Menu -->
                <nav class="mt-2">
                    <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
                        <!-- Add icons to the links using the .nav-icon class
                             with font-awesome or any other icon font library -->

                        <li class="nav-item">
                            <a href="?pg=produtos" class="nav-link">
                                <i class="nav-icon fas fa-barcode"></i>
                                <p>
                                    Produtos                                
                                </p>
                            </a>
                        </li>
                        <li class="nav-item">
                            <a href="?pg=servicos" class="nav-link">
                                <i class="nav-icon fas fa-handshake"></i>
                                <p>
                                    Serviços                                
                                </p>
                            </a>
                        </li>
                        <li class="nav-item">
                            <a href="?pg=contato" class="nav-link">
                                <i class="nav-icon fas fa-address-book"></i>
                                <p>
                                    Contato                                
                                </p>
                            </a>
                        </li>
                        <!-- drop-->
                        <li class="nav-item">
                            <a href="#" class="nav-link">
                                <i class="nav-icon fas fa-list-alt"></i>
                                <p>
                                    Departamentos
                                    <i class="fas fa-angle-left right"></i>
                                </p>
                            </a>
                            <ul class="nav nav-treeview">
                                <li class="nav-item">
                                    <a href="#" class="nav-link">
                                        <i class="fas fa-cocktail nav-icon"></i>
                                        <p>Comercial</p>
                                    </a>
                                </li>
                                <li class="nav-item">
                                    <a href="#" class="nav-link">
                                        <i class="fas fa-cogs nav-icon"></i>
                                        <p>Recursos Humanos</p>
                                    </a>
                                </li>
                                <li class="nav-item">
                                    <a href="#" class="nav-link">
                                        <i class="fas fa-cart-plus nav-icon"></i>
                                        <p>Compras e Estoque</p>
                                    </a>
                                </li>
                                <li class="nav-item">
                                    <a href="#" class="nav-link">
                                        <i class="fas fa-bus nav-icon"></i>
                                        <p>Operações</p>
                                    </a>
                                </li>
                                <li class="nav-item">
                                    <a href="#" class="nav-link">
                                        <i class="far fa-edit nav-icon"></i>
                                        <p>Marketing <small>e Propaganda</small></p>
                                    </a>
                                </li>
                                <li class="nav-item">
                                    <a href="#" class="nav-link">
                                        <i class="fas fa-charging-station nav-icon"></i>
                                        <p>Financeiro</p>
                                    </a>
                                </li>
                                <li class="nav-item">
                                    <a href="#" class="nav-link">
                                        <i class="fas fa-chess-queen nav-icon"></i>
                                        <p>Diretoria</p>
                                    </a>
                                </li>
                                
                            </ul>
                        </li>

                        <!--fim drop-->
  
                        <li class="nav-item">
                            <a href="#" class="nav-link">
                                <i class="nav-icon fas fa-calendar-alt"></i>
                                <p>
                                    Planejamento                                
                                </p>
                            </a>
                        </li>
                        <li class="nav-item">
                            <a href="#" class="nav-link">
                                <i class="nav-icon fas fa-bullhorn"></i>
                                <p>
                                    Mídia                                
                                </p>
                            </a>
                        </li>
                        <li class="nav-item">
                            <a href="#" class="nav-link">
                                <i class="nav-icon fas fa-chart-line"></i>
                                <p>
                                    Indices                                
                                </p>
                                <i class="nav-icon fas fa-ambulance"></i>
                            </a>
                        </li>
                        <li class="nav-item">
                            <a href="#" class="nav-link">
                                <i class="nav-icon fas fa-cash-register"></i>
                                <p>
                                    Faturamento                               
                                </p>
                            </a>
                        </li>
                        <li class="nav-item">
                            <a href="#" class="nav-link">
                                <i class="nav-icon fas fa-tools"></i>
                                <p>
                                    Configurações                               
                                </p>
                            </a>
                        </li>
                    </ul>
                </nav>
                <!-- /.sidebar-menu -->
            </div>
            <!-- /.sidebar -->
        </aside>

    </div> 